...
func TestPublicRoutes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			route:         "/api/v1/product/" + uuid.New().String(),
			expectedError: false,
			expectedCode:  404,
		},
		{
			description:   "get product by invalid ID (non UUID)",
			route:         "/api/v1/product/123456",
			expectedError: false,
			expectedCode:  500,
		},
	}

	// Define Fiber service.
	app := fiber.New()
	...
	for _, test := range tests {
		...
		// Perform the request plain with the service.
